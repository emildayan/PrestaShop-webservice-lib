...
class PrestaShopWebservice
{
    ...
    public function get($options)
    {
        ...
        if(App::isProduction()) return $this->getRequest($url, $options);
        return Cache::remember(md5($url), now()->addWeek(), function() use($url, $options) {
            return $this->getRequest($url, $options);
        });
    }

    private function getRequest($url, $options)
    {
